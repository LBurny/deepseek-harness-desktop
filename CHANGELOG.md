...
- Shell toasts now carry the app icon (appLogoOverride, 128x128 PNG shipped via the bundle resources — previously the toast XML had no image element at all, and the AUMID IconUri pointed at the exe which Windows does not render). The AUMID registry key now points its IconUri at the shipped PNG
- Clicking a toast opened the main window but buried it under the current foreground app (observed on machine B): with the Shell holding the foreground after the toast click, the running instance's SetForegroundWindow was denied by the foreground lock. show_main now does a brief always-on-top raise (topmost → focus → un-topmost) so the window lands on top of the z-order even when focus is denied
