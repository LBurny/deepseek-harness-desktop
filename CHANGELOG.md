...
- `/init` 命令结果行文案改为 "Prompt submitted to prepare for generating or maintaining the AGENTS.md file."（原 "Submitted the AGENTS.md init prompt as a collapsed context injection."——用户反馈原句偏实现细节）。dsh-command-init 插件 0.1.1 → 0.1.2，已安装实例下次启动经 preseed 文件同步自动更新
