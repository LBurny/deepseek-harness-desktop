...
# 用法：powershell -File scripts/release-local.ps1 [-Version 0.4.9] [-NotesPath <md 文件>] [-NotesOnly]
#       -NotesOnly 配合 -NotesPath：只重写两仓已发布 Release 的正文（PATCH），不碰安装包
#       资产——说明写错字/坏链接时用，避免为改几行字把 19MB exe 删了重传。
...
param(
    ...
    [switch]$NotesOnly, # 只重写两仓 Release 正文（PATCH），不碰安装包资产——改错别字/坏链接时用
...
# 上行 JSON 请求体必须发 UTF-8 **字节**，不能把字符串直接交给 Invoke-RestMethod。
# PS 5.1 在 ContentType 不带 charset 时按 ISO-8859-1 编码字符串体，所有非 ASCII 字符
# 变成 '?'（0.5.12 发布实踩：正文首行的 `English | [中文说明](…#中文说明)` 上线后是
# `English | [????](…#????)`，链接锚点也一起烂掉；PS 7 下同写法正常，因为
# package.json 走 powershell 5.1，手工用 pwsh 跑看不到）。顺带纠正旧注释：
# **5.1 的 ConvertTo-Json 不会把中文转 \uXXXX**，中文是原样进 JSON 的，编码责任
# 全在发送这一步。发字节体在 5.1/7 下都按原样上行（-ContentType 仍带 charset 标明）。
function Get-JsonBodyBytes([string]$Json) { , ([System.Text.Encoding]::UTF8.GetBytes($Json)) }

# 所有 JSON 正文的 HTTP 调用都走这里，别再让调用点自己拼 -Body/-ContentType。
function Send-GitHubJson([string]$Method, [string]$Uri, [hashtable]$Headers, [string]$UserAgent, [string]$Json) {
    Invoke-RestMethod -Method $Method -Uri $Uri -Headers $Headers -UserAgent $UserAgent `
        -Body (Get-JsonBodyBytes $Json) -ContentType 'application/json; charset=utf-8'
}
...
if ($SelfTest) {
    ...
    try {
        ...
        T '上行正文按 UTF-8 字节发送（0.5.12 的 ???? 回归哨兵）' {
            $json = (@{ body = (Read-NotesText $tmp) } | ConvertTo-Json)
            $bytes = Get-JsonBodyBytes $json
            Assert-Equal $bytes.GetType().FullName 'System.Byte[]' '应是字节数组，不能把字符串交给 Invoke-RestMethod'
            $back = [System.Text.Encoding]::UTF8.GetString($bytes)
            Assert-Equal $back $json 'UTF-8 往返应逐字相同'
            Assert-Equal $back.Contains('中文说明') $true '中文必须保真（字符串体在 PS 5.1 下会被 ISO-8859-1 打成 ?）'
        }
        T '所有 JSON 调用点都经字节助手（静态检查）' {
            # 唯一合法的正文调用是 -Body (Get-JsonBodyBytes …)；资产上传走 -InFile 不受影响。
            $src = [System.IO.File]::ReadAllText($PSCommandPath)
            $bad = @([regex]::Matches($src, '-Body\s+(?!\(Get-JsonBodyBytes)') | ForEach-Object { $_.Value })
            Assert-Equal $bad.Count 0 "发现未走字节助手的 JSON 请求体调用（会把非 ASCII 打成 ?）：$($bad -join ' / ')"
        }
...
if ($NotesOnly -and -not $NotesPath) { throw '-NotesOnly 只重写正文，必须一并给 -NotesPath' }
$headers = @{
...
# 会把 0.5.1 误配 0.5.11）。-NotesOnly 不碰资产，跳过整段。
$exe = $null; $shaFile = $null
if (-not $NotesOnly) {
...
foreach ($repo in $repos) {
    ...
    if ($release) {
        Write-Host "[$repo] Release $tag 已存在（id=$($release.id)）$(if ($NotesOnly) { '，只重写正文' } else { '，替换同名资产' })"
    } else {
        if ($NotesOnly) { throw "[$repo] Release $tag 不存在——-NotesOnly 只重写已发布正文，不建新 Release（避免建出无资产的空发布页）" }
        # 自定义说明时不走 generate_release_notes（那会追加 Full Changelog 链接并忽略自定义 body）
        ...
        $release = Send-GitHubJson -Method Post -Uri "$apiBase/releases" -Headers $headers -UserAgent $ua -Json ($createBody | ConvertTo-Json)
    ...
    if ($NotesPath) {
        ...
        Send-GitHubJson -Method Patch -Uri "$apiBase/releases/$($release.id)" -Headers $headers -UserAgent $ua -Json $patchBody | Out-Null
        Write-Host "[$repo] 已更新 Release 说明（来自 $NotesPath）"
    }
    if ($NotesOnly) { Write-Host "[$repo] 完成（-NotesOnly：资产未动）: $($release.html_url)"; continue }
