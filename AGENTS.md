...
- **手机端适配验证不能直连 dsh 端口**：mobile.css/mobile.js 只由代理注入，直连 `127.0.0.1:<dsh端口>` 的页面与手机看到的不是一回事（没有 项目/信息 标签、没有任何适配样式）——0.4.6 前在直连环境"验证"图标化白忙一轮。复刻手机环境=Playwright 390px 视口 + `addStyleTag/addScriptTag({path})` 注入同目录 mobile.css/mobile.js（mobile.js 挂观察器后有初始 `ensure()` 全量扫描，晚注入等价于 head 注入）；token 仅内存且日志脱敏，真走代理只能从托盘远程页读链接（用户用机期间别动 CUA）。**但"触发器内部有几个图形"这类问题与代理注入无关**（那是上游组件 + 我们 CSS 的结果），临时起一个裸 dsh 实例（副本运行时 + 临时 DSH_HOME + stdout 里的 token）在 390px 下直接量 DOM 就能验证，比在真机上靠肉眼猜可靠；空会话视图足以量到 composer——要进会话态就把本机真实 `storages/workspace.json`（格式合法：`unit/global{initialized,workspaceIds}/tables.workspaces`）拷进临时 home，UI 会直接列出工作区
