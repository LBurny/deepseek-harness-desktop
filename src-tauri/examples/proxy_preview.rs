...
//! "项目"标签。用法：
//!   set DSH_PORT=24203 && set DSH_TOKEN=<就绪行 ?token= 值> && cargo run --example proxy_preview
...
async fn main() {
    ...
    let launch_token: Arc<str> = std::env::var("DSH_TOKEN")
        .expect("set DSH_TOKEN=<dsh stdout 就绪行的 ?token= 值>")
        .into();
    let token: Arc<str> = generate_token().into();
    let (_tx, rx) = watch::channel(Some(Arc::new(DshCreds {
        port: dsh_port,
        token: launch_token,
    })));
